...
const LokaleOversigt = () => {
  ...
  // Funktion til at formatere lokale-id med punktum
          ...
          {bookings.length > 0 ? (
            <ul>
              {bookings.map((booking) => (
                <li key={booking.id}>
                  Lokale: {formatRoomId(booking.lokale_id)}, Dato: {dayjs(booking.start_time).format('DD/MM/YYYY')}, Tid: {dayjs(booking.start_time).format('HH:mm')} - {dayjs(booking.end_time).format('HH:mm')}
